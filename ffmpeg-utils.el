...
;; Package-Version: 20230212.55
;; Package-Commit: 7ae311d235514b7b5bf12dc2c214856856c2c411
;; Version: 0.1
;; Keywords: multimedia
       ...
       (cond
        ((featurep 'alert)
         (let ((msg "Emacs ffmpeg-utils.el process finished."))
           (cl-case system-type
             (darwin
              (do-applescript (format "say \"%s\"" msg))))
...
00:17:23 -- 00:21:45."
  ...
  (let* ((processed-input-filename (substring input-filename 0 (string-match-p "?" input-filename)))
         (processed-output-filename (substring output-filename 0 (string-match-p "?" output-filename)))
         (input-type (file-name-extension processed-input-filename))
         (output-type (file-name-extension processed-output-filename)))
    (if (not (string-equal "" output-type))
        ...
        (if (string-equal output-type input-type) ; if output extension is same as input
            processed-output-filename             ; keep output original extension
          (if (yes-or-no-p "Whether keep output video format extension? ")
              processed-output-filename
            (setq processed-output-filename
                  (format "%s.%s" (file-name-sans-extension processed-output-filename) input-type))))
      ;; output filename has NOT specific video file extension.
      (setq processed-output-filename (format "%s.%s" processed-output-filename input-type)))
